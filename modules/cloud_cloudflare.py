#!/usr/bin/env python3
# modules/cloud_cloudflare.py — Cloudflare exposure and origin-candidate assessment for CLVX
...
def is_cloudflare_ip(ip: str) -> bool:
    try:
        ...
        return False
    for net in CF_RANGES_V4 + CF_RANGES_V6:
        try:
...
class CloudflareExposure(BaseModule):

    NAME        = "cloud/cloudflare"
    DESCRIPTION = "Cloudflare exposure and origin-confidence assessment"
    REFERENCES  = [
    ...
    def run(self) -> list:
        ...
        print_section(f"Cloudflare Exposure — {domain}")

        print_status("Confirming Cloudflare presence...", "run")
        cf_active = self._confirm_cloudflare(domain, client)
        if cf_active:
            print_status(
                f"{Colors.YELLOW}Cloudflare detected{Colors.RESET} — starting origin-candidate assessment", "waf"
            )
                ...
                if verify:
                    ...
                    if verified:
                        print(f"    {Colors.BOLD}{Colors.RED}[VERIFIED]{Colors.RESET} "
                              f"HTTP route matched with Host header; this is not proof of origin ownership.")
                        findings.append(self._finding(
                            "MEDIUM", "Origin route match observed",
                            ip, f"Host-header routing matched the target | Sources: {src_str}. This is not proof of direct origin ownership."
                        ))
                    else:
                        findings.append(self._finding(
                            "MEDIUM", "Origin IP Candidate (Unverified)",
                            ip, f"Sources: {src_str}. Candidate requires further validation."
                        ))
                else:
                    findings.append(self._finding(
                        "MEDIUM", "Origin IP Candidate",
                        ip, f"Sources: {src_str}. Candidate requires further validation."
                    ))

        print()
        print_status(
            f"Cloudflare exposure assessment complete. "
