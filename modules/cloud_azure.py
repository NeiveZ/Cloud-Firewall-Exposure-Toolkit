        ...
        for fqdn in targets:
            try:
                ...
                if any(s in fqdn for s in [".azurewebsites.net", ".blob.core.windows.net"]):
                    print(f"  {Colors.YELLOW}[NOT RESOLVED]{Colors.RESET} {Colors.DARK_GRAY}{fqdn}{Colors.RESET} — stale/orphaned Azure DNS cannot be confirmed from NXDOMAIN alone")
                    findings.append(self._finding(
                        "INFO", "Azure DNS candidate not resolved",
                        fqdn,
                        "NXDOMAIN observed. This is not sufficient evidence of a claimable Azure resource; validate CNAME/resource ownership separately."
                    ))
        return findings
